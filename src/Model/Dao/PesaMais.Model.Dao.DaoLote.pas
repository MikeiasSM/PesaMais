...
begin
  FConnection.StartTransation;
  try
    FConnection.PrepareStatement('INSERT INTO LOTE (DESCRICAO) VALUE (:DESCRICAO)');
...
begin
  FConnection.StartTransation;
  try
    FConnection.PrepareStatement('UPDATE LOTE SET DESCRICAO = :DESCRICAO WHERE ID_LOTE = :ID_LOTE');
    FConnection.FDQuery.Params.ParamByName(':DESCRICAO').AsString := pLote.Descricao;
    FConnection.FDQuery.Params.ParamByName(':ID_LOTE').AsInteger := pLote.Id_lote;
    FConnection.ExecSQL;
    FConnection.Commit;
